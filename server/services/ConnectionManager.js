...
//     // Crear promesa para esta inicialización
//     const initPromise = new Promise((resolve, reject) => {
//       this._doInitPool(serverKey, customConfig).then(resolve).catch(reject);
//     });
...
//           logger.info(
//             `Esperando a que termine el cierre del pool en curso para ${serverKey}...`
//           );
...
//       // Verificar estado del pool antes de adquirir
//       if (!this.pools[serverKey] || this.pools[serverKey]._draining) {
//         throw new Error(
//           `El pool para ${serverKey} está en estado inválido o drenando`
//         );
//       }

//       // Adquirir conexión del pool
//       const connection = await this.pools[serverKey].acquire().timeout(30000); // 30s timeout
...
//       // Errores específicos que requieren reinicialización del pool
//       if (
//         error.message.includes("draining") ||
//         error.message.includes("cannot accept work") ||
...
//           // Segundo intento después de reinicializar
//           logger.info(
//             `Reintentando obtener conexión después de reinicializar pool para ${serverKey}`
//           );
//           return await this.getConnection(serverKey);
//         } catch (reinitError) {
...
//       logger.error(
//         `Error al obtener conexión para ${serverKey}: ${error.message}`
//       );
//       throw error;
//     }
//   }
...
//   /**
//    * Cierra un pool de forma segura, con manejo de estados
//    * @param {string} serverKey - Clave del servidor
...
//       // Marcar el pool como en proceso de cierre
...
//       // Establecer un timeout para el caso de que drain se quede bloqueado
//       const drainTimeout = setTimeout(() => {
//         logger.warn(
//           `Timeout al intentar drenar pool para ${serverKey}, forzando cierre`
//         );
//         try {
//           if (this.pools[serverKey] === pool) {
//             delete this.pools[serverKey];
//           }
//           this._closingPools.delete(serverKey);
//         } catch (timeoutError) {
//           logger.error(
//             `Error al forzar cierre de pool: ${timeoutError.message}`
//           );
//         }
//       }, 15000); // 15 segundos máximo para drenar

//       try {
//         // Cerrar pool
//         await pool.drain();
//         clearTimeout(drainTimeout); // Cancelar timeout si termina correctamente
//         await pool.clear();
...
//         return true;
//       } catch (error) {
//         clearTimeout(drainTimeout); // Asegurar que el timeout se limpia en caso de error

//         // Si hay error, intentar recuperar
//         logger.error(
//           `Error al cerrar pool para ${serverKey}: ${error.message}`
//         );
...
//         } catch (cleanupError) {
//           logger.error(
//             `Error durante limpieza forzada: ${cleanupError.message}`
//           );
//         }
...
//       // Asegurar limpieza incluso en errores externos
...
//                 );
//                 ConnectionService.releaseConnection(testConnection);
